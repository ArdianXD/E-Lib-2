...
--
-- Host: localhost:3306
-- Generation Time: Jun 13, 2022 at 07:13 PM
...
CREATE ALGORITHM=UNDEFINED DEFINER=`stylecod_haruman`@`%` SQL SECURITY DEFINER VIEW `daftar_user`  AS SELECT `user`.`nama` AS `nama`, `user`.`username` AS `username`, `user`.`email` AS `email`, `user`.`role` AS `role` FROM `user` ;
